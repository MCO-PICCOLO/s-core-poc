...
const PERSISTENCY_SCHED_PREFIX: &str = "Schedule";
...
impl ActionControllerManager {
    ...
    async fn execute_model_action(
     ...
     async fn handle_realtime_sched(&self, sched: &str) -> Result<()> {
        ...
        let sched_str = common::persistency::get(&format!("{}/{}", PERSISTENCY_SCHED_PREFIX, sched)).await?;
    ...
    pub async fn trigger_manager_action(&self, scenario_name: &str) -> Result<()> {
        ...
        // For update/rollback: send new schedule to timpani-o BEFORE restarting
        // the container. This guarantees the updated schedule parameters are
        // available in timpani-o before the container reconnects to timpani-n.
        // For reschedule: only send schedule, no container restart at all.
        if action == "update" || action == "rollback" || action == "reschedule" {
        ...
        for mi in package.get_models() {
            ...
            // "reschedule" only updates timing parameters in timpani-o,
        ...
        // For launch/terminate: send schedule after model actions (original behavior).
        // update/rollback already sent schedule above.
        // reschedule already sent schedule above (before this loop).
        if action != "update" && action != "rollback" && action != "reschedule" {
            if let Some(sched) = package.get_schedule() {
                self.handle_realtime_sched(sched).await?;
            }
