...
async fn launch_manager(
    ...
    match manager.initialize().await {
        Ok(_) => {
            ...
            // Register with API server (retry with exponential backoff)
            const MAX_RETRIES: u32 = 5;
            const INITIAL_DELAY_SECS: u64 = 2;
            let mut registered = false;
            for attempt in 1..=MAX_RETRIES {
                match sender
                    .register_with_api_server(registration_request.clone())
                    .await
                {
                    Ok(_) => {
                        println!("Successfully registered with API server (attempt {})", attempt);
                        registered = true;
                        break;
                    }
                    Err(e) => {
                        if attempt < MAX_RETRIES {
                            let delay_secs = INITIAL_DELAY_SECS * (1 << (attempt - 1));
                            eprintln!(
                                "Failed to register with API server (attempt {}/{}): {:?}. Retrying in {}s...",
                                attempt, MAX_RETRIES, e, delay_secs
                            );
                            tokio::time::sleep(tokio::time::Duration::from_secs(delay_secs)).await;
                        } else {
                            eprintln!(
                                "Failed to register with API server after {} attempts: {:?}",
                                MAX_RETRIES, e
                            );
                        }
                    }
            ...
            if !registered {
                eprintln!("Proceeding without successful API server registration");
            }
