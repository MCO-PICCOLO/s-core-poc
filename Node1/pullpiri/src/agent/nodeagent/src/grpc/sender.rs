...
impl NodeAgentSender {
    ...
    ///
    /// Attempts the connection up to `max_connect_attempts` times before
    /// giving up, so callers get a single consolidated error on persistent
    /// transport failures.
    pub async fn register_with_api_server(
        ...
        const CONNECT_RETRY_DELAY_SECS: u64 = 1;

        let mut last_err = Status::unknown("No connection attempt made");
        for attempt in 1..=MAX_CONNECT_ATTEMPTS {
            match ApiServerConnectionClient::connect(addr.clone()).await {
                Ok(mut client) => {
                    return client
                ...
                Err(e) => {
                    last_err = Status::unknown(format!(
                        "Failed to connect to API server (attempt {}): {}",
                        attempt, e
                    ));
