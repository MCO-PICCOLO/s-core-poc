                                          ...
                                          Response* reply)
{
    ...
    // Deep-copy the NodeSchedInfoMap so that our stored tasks pointers are
    // independent of GlobalScheduler's memory (which is freed on next clear()).
    NodeSchedInfoMap deep_copy;
    for (const auto& entry : node_sched_map) {
        const std::string& node_id = entry.first;
        const sched_info_t& src = entry.second;
        sched_info_t dst;
        dst.num_tasks = src.num_tasks;
        dst.tasks = nullptr;
        if (src.num_tasks > 0 && src.tasks) {
            dst.tasks = static_cast<sched_task_t*>(
                malloc(sizeof(sched_task_t) * src.num_tasks));
            if (dst.tasks) {
                memcpy(dst.tasks, src.tasks, sizeof(sched_task_t) * src.num_tasks);
            } else {
                dst.num_tasks = 0;
            }
        }
        deep_copy[node_id] = dst;
        // Record that this node now belongs to this workload (most recent wins).
...
std::string SchedInfoServiceImpl::GetWorkloadForNode(const std::string& node_id) const
{
    std::shared_lock<std::shared_mutex> lock(sched_info_mutex_);
    auto it = node_to_workload_.find(node_id);
    if (it != node_to_workload_.end()) {
        return it->second;
    }
    return {};
}
...
std::shared_ptr<NodeConfigManager> SchedInfoServiceImpl::GetNodeConfigManager() const
{
    return node_config_manager_;
}
 
std::shared_ptr<NodeConfigManager> SchedInfoServer::GetNodeConfigManager() const
{
    return service_.GetNodeConfigManager();
}

std::string SchedInfoServer::GetWorkloadForNode(const std::string& node_id) const
{
    return service_.GetWorkloadForNode(node_id);
}
