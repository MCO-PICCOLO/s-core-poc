...
use crate::activities::components::{
    ...
    RearCamera, SteeringController, TrajectoryVisualizer,
};
#[cfg(feature = "com_mw")]
...
pub fn agent_assignments() -> HashMap<AgentId, Vec<(WorkerId, Vec<ActivityIdAndBuilder>)>> {
    ...
    // Worker 45 runs the rear-camera DDS publisher on the primary agent.
    let w45: WorkerAssignment = (
        45.into(),
        vec![(9.into(), Box::new(|id| RearCamera::build(id)))],
    );
    ...
    let assignment = [(100.into(), vec![w40, w41, w42, w43, w44, w45])].into_iter().collect();
...
pub fn activity_dependencies() -> ActivityDependencies {
    ...
    let dependencies = [
        ...
        (8.into(), vec![5.into()]),
        // RearCamera — independent DDS publisher, no FEO predecessors
        (9.into(), vec![]),
    ];
