...
/// Optional observer for task-chain boundaries.
...
pub(crate) struct Scheduler {
    ...
    /// Optional observer invoked at the start and end of each task chain.
    cycle_observer: Option<Box<dyn CycleObserver>>,
}
    ...
    pub(crate) fn run(&mut self) {
        ...
        loop {
            ...
            if let Some(observer) = self.cycle_observer.as_mut() {
                if let Err(err) = observer.on_cycle_start() {
                    error!("Failed to start cycle observer: {}", err);
                    self.shutdown_gracefully("Failed to start cycle observer.");
                    return;
                }
            }
            ...
            if let Some(observer) = self.cycle_observer.as_mut() {
                if let Err(err) = observer.on_cycle_end() {
                    error!("Failed to stop cycle observer: {}", err);
                    self.shutdown_gracefully("Failed to stop cycle observer.");
                    return;
                }
            }
